...
function detectProvider(lines, text) {
  ...
  const likely = lines.find(l => /s\.r\.l|srl|s\.a\.|sa\b|sociedad|electric|electro|ferreter|materiales|industrial|servicios/i.test(l));
  return likely || '';
}
  ...
  lines.forEach(line => {
    ...
    for (const known of knownItemPatterns) {
      for (const pattern of known.patterns) {
        if (pattern.test(line)) { matchedId = known.id; break; }
      }
      if (matchedId) break;
    }
    if (!matchedId) {
      const idMatch = line.match(/^(\d{1,2})[\s.\-]/);
      if (idMatch) { const pid = parseInt(idMatch[1]); if (pid >= 1 && pid <= 19) matchedId = pid; }
    }
    extracted.push({
...
function detectQuality(text) {
  ...
  if (/schneider|abb|siemens|genrod|sica|jeluz|cambre|phoenix/i.test(t)) return 'premium / reconocida';
...
function buildParsed(text) {
  ...
  const telefono = extractField(normalized, [/(?:tel[eé]fono|tel|whatsapp)[:\s]+([^\n]+)/i, /(\+?54\s?9?\s?\d[\d\s-]{7,})/i]);
  const email = extractField(normalized, [/(?:e-?mail|correo)[:\s]+([^\s\n]+)/i, /([\w.-]+@[\w.-]+\.[A-Za-z]{2,})/i]);
  const direccion = extractField(normalized, [/(?:domicilio|direcci[oó]n)[:\s]+([^\n]+)/i]);
...
export default async function handler(req, res) {
  ...
  if (req.method !== 'POST') return res.status(200).json({ ok: true, mensaje: 'function viva' });
  ...
  try {
    ...
    if (fileType === 'pdf' || raw.toString('ascii', 0, 10).includes('%PDF')) {
      try { text = await tryPdfText(raw); if (text) source = 'pdf-text'; } catch {}
    }

    if (!text && (fileType === 'image' || fileType === 'pdf')) {
      try { text = await tryOCR(raw); if (text) source = 'ocr'; } catch {}
    }

    if (!text && fileType === 'unknown') {
      try { text = raw.toString('utf-8'); if (text) source = 'raw-text'; } catch {}
    }

    return res.status(200).json({ ok: true, parsed: buildParsed(text), debug: { source, extractedChars: text.length, fileType } });
  } catch (error) {
    return res.status(200).json({ ok: true, parsed: buildParsed(''), debug: { error: String(error.message || error) } });
  }
}
